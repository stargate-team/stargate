...
public class ServiceStateModel {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(ServiceStateModel.class);
  ...
  /**
   * Query the service state. Note: This is a non-blocking operation.
   * 
   * @return the state
   */
  ...
  public boolean isInState(Service.STATE proposed) {
    LOG.debug("State = {}, proposed = {}, eq = {}", state, proposed,
        state.equals(proposed));
   ...
   * @throws ServiceStateException if the service state is different from the
   *           desired state
   */
  public void ensureCurrentState(Service.STATE expectedState) {
    if (state != expectedState) {
      throw new ServiceStateException(
          name + ": for this operation, the " + "current service state must be "
   ...
   *
   * <p>
   * Using an array kills off all branch misprediction costs, at the expense of
   * cache line misses.
   *
  ...
  /**
   * Get the state text as the toString() value.
   * 
