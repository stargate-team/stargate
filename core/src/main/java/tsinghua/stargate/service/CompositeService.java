...
public class CompositeService extends AbstractService {
   ...
   * close started services (which assumes that the service implementations may
   * not handle the stop() operation except when started. Irrespective of this
   * policy, if a child service fails during its init() or start() operations,
   ...
   * 
   * @return a list of child services at the time of invocation - added services
  ...
  /**
   * If the passed object is an instance of {@link Service}, add it to the list
   * of services managed by this {@link CompositeService}
   * 
   * @param object
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    debug("{}: initing services (size: {})", getName(), services.size());
  ...
  protected void serviceStart() throws Exception {
    List<Service> services = getServices();
    debug("{}: starting services (size: {})", getName(), services.size());
    for (Service service : services) {
  ...
  protected void serviceStop() throws Exception {
    ...
    debug("{}: stopping services (size: {})", getName(), numOfServicesToStop);
   ...
   * @throws RuntimeException the first exception raised during the stop process
   *           -<i>after all services are stopped</i>
   */
