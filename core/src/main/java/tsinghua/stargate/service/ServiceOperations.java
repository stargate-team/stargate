...
public final class ServiceOperations {
   ...
   * at warn level. (but not Throwables). This operation is intended to be used
  ...
  public static Exception stopQuietly(Logger log, Service service) {
    try {
      LOG.debug("Quietly stopping {}", service);
      stop(service);
    } catch (Exception e) {
   ...
   * including a notification serve that is robust against changes to the list
  ...
  public static class ServiceListeners {
    ...
    /**
     * Thread-safe addition of a new listener to the end of a list. Attempts to
     * re-register a listener that is already registered will be ignored.
     * 
     * @param l listener
     */
    ...
    /**
     * Change to a new state and notify all listeners. This method will block
     * until all notifications have been issued. It caches the list of listeners
     * before the notification begins, so additions or removal of listeners will
     * not be visible.
     * 
    ...
    public void notifyListeners(Service service) {
      ...
      synchronized (this) {
        callbacks =
            listeners.toArray(new ServiceStateChangeListener[listeners.size()]);
      }
